...
models:
  comfylocal.myworkflow:
    provider: comfylocal
    workflowdir: "comfyui_workflow_templates"   # 工作流 JSON 文件目录
    node_mappings:
      prompt:                                    # OPSV 参数名
...
opsv comfy-node-mapping comfyui_workflow_templates/ref2.json

# 保存到文件
opsv comfy-node-mapping comfyui_workflow_templates/ref2.json -o node_mapping.yaml
```
...
`workflowdir` 指向一个目录，里面放 `.json` 格式的 ComfyUI 工作流文件。文件按引用图片数量命名：

```
comfyui_workflow_templates/       ← workflowdir 指向这里
...
`opsv comfy` 编译时会根据文档中 `refs` 的图片数量自动匹配最合适的模板：
- 精确匹配 `ref{N}.json`（N = ref 图片数量）
- 无精确匹配时取最接近的下限（丢弃多余 ref）
- 或最接近的上限（留空槽位用默认值）
...
| 工作流来源 | 本地 `workflowdir` 目录中的 `.json` 文件 | RunningHub 服务端的 `workflowId` |
| node_mappings | 必填 | 必填 |
...
```bash
# 添加本地 ComfyUI 工作流（auto api_url 默认 127.0.0.1:8188）
opsv api-setup --add-model '{
  "modelKey": "comfylocal.txt2img",
  "config": {
    "provider": "comfylocal",
    "workflowdir": "comfyui_workflow_templates",
    "node_mappings": {
