...
status: drafting | draft | pending_sync | approved
visual_brief: >
  视觉描述简述
visual_detailed: >
  视觉详细特征描述
...
```
drafting → draft → [渲染] → [approve] → pending_sync → [Agent对齐] → approved
```
...
| `approved` | 完全就绪，允许下游依赖引用 |

> **`pending_sync` 设计原则**：Approve 时系统只覆盖 `prompt_en`（实际发送的 prompt），不自动修改翻译字段（`visual_detailed`/`visual_brief`）和 `refs`——这些字段需要 Agent 根据 `prompt_en` 手动对齐。`pending_sync` 作为正式枚举值，DependencyGraph 将其视为"未就绪"，阻断下游 Circle 执行，确保对齐完成后才能进入 `approved`。
...
### 7.1.1 文档校验规则（`opsv validate`）

| 规则 | 条件 | 级别 | 说明 |
|------|------|------|------|
| 1 | `status: approved` 但无 `## Approved References` | error | approved 资产必须有定档图 |
| 2 | 有 Approved References 但 status 非 approved/pending_sync | error | status 与实际内容不一致 |
| 3 | `pending_sync` + `visual_detailed`/`visual_brief`/`refs` 缺失 | warning | 需 Agent 根据 `prompt_en` 完成对齐 |
| 3b | `pending_sync` + 所有字段已填充 | warning | 提醒确认对齐后改为 `approved` |
| 4 | `approved` + `prompt_en` 与 `visual_detailed` 不一致 | warning | 提示 `prompt_en` 已被实际 prompt 覆盖，需确认 `visual_detailed` 是否同步 |
...
4. **Approve**（自动回写 + pending_sync）：
   - 向源 Markdown 文档的 `## Approved References` 区追加引用（图片保持在原队列目录中，通过相对路径引用）
   - **回写策略**（`writebackFromTask`）：
     1. **覆盖 `prompt_en`**：始终覆盖为 task JSON 中实际发送的 prompt
     2. **同步 `## Design References`**：从 task JSON 提取参考图 URL，替换该区域
     3. **追加 `reviews[]` 条目**：指向 task JSON 路径 + 模型 + 尺寸
     4. **`status → pending_sync`**：标记为待对齐（无 task JSON 时 fallback 为 `approved`）
   - 自动 `git commit`
   - **Agent 对齐步骤**（从 `pending_sync` → `approved`）：
     - `opsv validate` 检出哪些字段需要对齐
     - 根据 `prompt_en` 更新 `visual_detailed` / `visual_brief` / `refs`
     - 手动将 `status` 改为 `approved`
5. **Draft**：
...
| 区域 | 职责 | 写入时机 |
|------|------|---------|
| `## Design References` | **输入侧**：生成本实体时使用的参考图 | Approve 时从 task JSON 同步 |
| `## Approved References` | **输出侧**：本实体定档后的正式参考图 | Approve 时追加 |
...
- **pending_sync 资产**：Approve 回写后进入 `pending_sync`，阻断下游 Circle，Agent 需完成 `visual_detailed`/`visual_brief`/`refs` 对齐后改为 `approved`
- **全部 Approved（✅）**：执行 `opsv circle manifest` 固化快照，允许晋升下一 Circle
- **部分 Approved（⏳）**：包含 `pending_sync` 资产时仍显示 ⏳，需 Agent 完成对齐
...
| **v0.6.4 新增** | `pending_sync` 状态：Approve 回写后进入 pending_sync，Agent 对齐后方可 approved，阻断下游 Circle |
| **v0.6.4 新增** | Approve 回写策略：只覆盖 `prompt_en` + 同步 Design References + review 指向 task JSON |
