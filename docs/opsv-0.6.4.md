...
│   │   │       ├── queue.json  # provider 批次队列列表以及api log，如果列表需要更新，则队列序号更新（新队列目录），列表不更新，仅仅是断连重试，或再次运行，不创建新的目录,但创建资产依然按照全局序号递增。
│   │   │       ├── shot_01_1.png  
│   │   │       ├── shot_01_2.png   # 重复运行保存资产序号递增
│   │   │       ├── shot_01_1.json  # 从queue.json 拆分出 单个任务的api请求格式，可以通过   
│   │   │       └── shot_02_1.png
│   │   └── siliconflow/
│   │       └── queue_1/
...
opsv queue compile <jobs.json> --<provider.model> [--circle <name>]
# 例: opsv queue compile imagen_jobs.json --volcengine.seadream-5.0-lite --circle zerocircle_1
# 例: opsv queue compile jobs.json --volcengine.seadream-5.0-lite --siliconflow.qwen-image --circle zerocircle_1

# 一次性顺序执行最新 batch 下的任务
opsv queue run --<provider.model> [--file <json>...] [--circle <name>] [--retry]
# 例: opsv queue run --volcengine.seadream-5.0-lite --file shot_01.json --circle zerocircle_1
# 例: opsv queue run --siliconflow.qwen-image --retry
```
...
1. `queue compile <jobs.json> --<provider.model> --circle <name>`
2. 计算 `jobs.json` 的 sha256 sourceHash
3. 读取 `api_config.yaml` 获取指定 model 的 api_url、defaults、参数防御规则
4. 检查 jobs.json 中各 job 的 type 是否与 model.type 匹配，不匹配则跳过
5. 构建完整的 API 请求体（含 model、prompt、defaults、reference 映射、frame_ref 映射）
6. **compile 必然创建新 batch**：`queue_{N+1}/`
   - `queue_N` 语义 = "第 N 次 compile 的结果"
   - run 默认执行最新 batch
7. 每个 job 写入 `{jobId}.json`（可直接发送的 API 请求体）
8. 生成 `queue.json` 只读索引 + `compile.log`

### 5.2 执行流程细节

1. `queue run --<provider.model> [--file <json>...] [--retry]`
2. 定位最新 `queue_N/` 目录
3. 扫描 `.json` 任务文件，过滤规则：
...
- 修改 `prompt` 字段
- 运行 `opsv queue run --<provider.model> --file shot_01_v2.json`
- 生成 `shot_01_v2_1.png`
...
| `volcengine` | `seedance-2.0` | 视频 | `VOLCENGINE_API_KEY` | **新版 Content Generation API**，支持多模态（图/视频/音频参考，最多9张图） |
...
| `volcengine` | `2K` → `1920x1080`，`2K-Square` → `1440x1440`，默认 `1280x720`；Seedance 2.0 自动构建 `content` 数组（含 text/image_url/video_url/audio_url） |
...
| `frame_ref` | 视频任务的首帧/尾帧根据 Provider 映射到 `first_frame`/`last_frame` (旧版) 或 `content` 数组中的 `image_url` + `role: reference_image` (Seedance 2.0) |
...
3. `opsv queue compile <jobs.json> --<provider.model> --circle <name>` 编译为可直接执行的 `.json`
4. `opsv queue run --<provider.model> --circle <name>` 一次性顺序执行
5. `opsv review` 审阅产出
6. **Agent 迭代**：复制 `.json` → 修改参数 → `opsv queue run --<provider.model> --file <json>` 重新执行
...
| **v0.6.4 新增** | `--<provider.model>` CLI 格式，精确指定模型 |
