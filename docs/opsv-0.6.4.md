...
| `volcengine` | `seedance-2.0` | 视频 | `VOLCENGINE_API_KEY` | **Provider 尚未适配**，enable=false |
...
| `volcengine` | `2K` → `1920x1080`，`2K-Square` → `1440x1440`，默认 `1280x720` |
| `minimax` | `negative_prompt` 拼接进 prompt 文本后从请求体删除；默认启用 `prompt_optimizer: true` |
| `reference 映射` | 通用 `references` 数组根据 Provider 映射到正确字段：`image` (SiliconFlow/Volcengine)、`subject_reference` (Minimax)、`image_url` (Volcengine video) |
| `frame_ref` | 视频任务的首帧/尾帧根据 Provider 映射到 `first_frame`/`last_frame` 或 `image` 字段 |
