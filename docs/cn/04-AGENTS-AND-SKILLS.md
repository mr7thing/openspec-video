# OpsV Agent & Skill 体系 (Agents & Skills)
...
├──────────────────────────────────────────────────┤
│ CLI — "三步 Circle Queue"                         │
│ imagen / animate → queue compile → queue run     │
└──────────────────────────────────────────────────┘
```
...
| **Runner-Agent** | `Runner-Agent.md` | 疾走特遣：任务编译、Circle Queue 分发、管线监控、产物归档 | `opsv-animator`, `animation-director`, `opsv-enlightenment` |
...
| 阶段 | 输入 | 输出 | 是否生成文件 |
|------|------|------|------------|
| Phase 1: 概念发散 | 一句歌词/模糊概念 | 3 个差异化故事方案 | ❌ 仅文本 |
...
**长镜头继承**：通过 `@FRAME:shot_N_last` 自动截取前一视频的尾帧作为下一镜头的首帧。

---

### 6. `animation-director` — 动画编导艺术
...
**Approve / Draft 双态流程**：
- **Approve**：转入 Runner，回写 `## Approved References` + `status: approved`，后续 `--skip-approved` 自动跳过
...
2. **任务编排**：`opsv imagen` → `opsv queue compile` → `opsv queue run`
...
> *"Agent 是灵魂，规范 Skill 是骨架，创意 Skill 是血肉，Circle Queue 是神经。"*
