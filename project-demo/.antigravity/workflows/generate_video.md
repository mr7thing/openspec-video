...
## Prerequisites
- **Storyboards**: You must have generated images in `project-demo/artifacts/` (or `videospec/stories/storyboard/`).
- **Script**: `Script.md` should contain `Camera` directives (e.g., "Slow zoom in").

## Steps

### 1. Verification
1. Check `project-demo/artifacts/` for source images (e.g., `shot_1.png`).
...
1. Open `project-demo/__antigravity/workflows/executor.md`.
2. Follow the "Browser Interaction (Veo 3.1)" section.
   ...
   - Save the resulting video (mp4) to `project-demo/artifacts/videos/`.

### 4. Post-Processing
1. Rename files to match shot IDs (e.g., `shot_1_video.mp4`).
2. Update `project-demo/artifacts/manifest.md` (if it exists) to log the new video assets.
