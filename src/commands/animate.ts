...
export function registerAnimateCommand(program: Command, VERSION: string) {
    program
        ...
        .option('--circle <name>', '目标 Circle (默认 auto: 自动推断依赖图末端环)', 'auto')
        .action(async (options) => {
            try {
                ...
                const jobs = await generator.generateAnimationJobs(options.circle);

                if (jobs.length > 0) {
                    console.log(`✅ ${jobs.length} video job(s) compiled.`);
                    console.log('   Next step: opsv queue compile opsv-queue/<cycle>/video_jobs.json --provider <name>');
                } else {
