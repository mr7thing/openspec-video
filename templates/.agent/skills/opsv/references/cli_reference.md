...
**ComfyUI Local**：加载本地 workflow JSON 模板，通过 frontmatter 中的 `node_mapping` 将 OpsV 参数注入对应节点。支持 `seed: random`（自动替换为随机自然数）。
...
**运行纪律 — 严禁随意中断已提交任务**：
- 任务一旦提交到 API 后**严禁 kill session**——中断不会退回额度，error 605 已证明余额不可回收
- 让当前批次自然跑完，拿到产出后再根据结果做决策或迭代
- 重复提交同一任务也会重复消耗额度，提交前确认 prompt 正确
...
2. 从 frontmatter 的 `refs` 字段重建依赖图（`refs` 为结构化数组 `[{ id, type }]`，提取 `ref.id` 解析依赖）
...
| Review Approve 后 | `opsv circle refresh` | 批准状态变更，可能解锁下游层级 |
| Review Draft 后 | `opsv circle refresh` | 批准状态回退，必须阻断下游 Circle |
| 迭代重生成（修改 prompt 重跑） | `opsv circle refresh` | 确认新迭代已纳入 manifest |
...
```
opsv-queue/                                    # 渲染产物目录
└── videospec_circle1/                        # 一个 Circle 批次（含所有层级）
    ├── _manifest.json                        # 全部 Layer: zerocircle, firstcircle, ...
...
# videospec_circle2/ 仅在依赖层级结构变化时由 opsv circle create 新建
```
...
4. **`opsv circle refresh`**: 确认依赖关系。如层级结构未变，继续在同一 Circle 目录编译下一层。
5. **`opsv imagen --model <provider.model>`**: 编译 ZeroCircle 图像任务。
6. **`opsv run <paths...>`**: 执行渲染 → `opsv review` 审查批准。
