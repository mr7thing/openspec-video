...
**refs 表达的是「生成此资产所需的视觉输入依赖」，不是剧情关系、上下文关系、或归属关系。**

这是 Circle 拓扑排序能否成立的根本前提——refs 必须构成有向无环图（DAG）。把剧情或归属关系塞进 refs，几乎必然产生循环依赖。
...
只有三个答案都是「视觉输入依赖」，才该进 refs。

**正确的依赖方向**（自底向上）：

| 层级 | 资产类型 | refs 应包含 | refs 不应包含 |
|------|---------|------------|--------------|
| 第 0 层 | 风格参考、底图、素材库 | 空 / 仅外部素材 | 任何角色 / 场景 / 分镜 |
| 第 1 层 | 角色定档（`@hero`） | 第 0 层风格参考 | 其他角色、场景、分镜 |
| 第 2 层 | 场景定档（`@temple`） | 第 0 层风格参考 | 角色（即使该场景有角色出现） |
| 第 3 层 | 分镜画面（`@shot_01`） | 用到的第 1 层角色 + 第 2 层场景 | 其他分镜 |
| 第 4 层 | 分镜视频（shotlist） | 第 3 层分镜的帧 | — |
...
# ❌ 错误：场景定档 refs 引用了将出现在场景里的角色
# scenes/temple.md
refs:
  image:
    "@hero":   [...]     # 画庙宇时不需要先有英雄；英雄出场是分镜的事

# ❌ 错误：角色之间互相引用（A 引用 B，B 引用 A）
# elements/sister.md
refs:
  image:
    "@brother": [...]    # 两个独立角色就是独立定档，不该互相依赖

# ✓ 正确：分镜画面引用其需要表现的角色与场景
# shots/shot_01.md
refs:
  image:
    "@hero":    [...]
...
**剧情关系应该写在哪里**：
- 故事大纲（`stories/story.md`）—— 自由文本叙述
- 分镜 prompt 正文 —— `@hero 走入 @temple` 这种描述
- 不该写进 element / scene 的 refs

### Prompt 写作指导 ⚠️

**prompt 是给图像/视频生成模型的指令，不是叙事文本。**

它只服务一件事：**告诉模型当前要生成的画面/视频里有什么、长什么样、什么风格**。

**核心规则**：

1. **描述画面而非剧情**：prompt 写"画面里的元素"，不写"故事发生了什么"
2. **`@id` 必须是画面要表现的元素**：每个 `@` 引用都对应到生成结果里能被人看到/听到的视觉/听觉对象
3. **不要在 prompt 里写"背景故事"、"角色心情"、"前情提要"**：这些不会生成出来，只会占用 token
4. **`@FRAME:*` 用于视频生成的起止帧**：仅 shotlist 使用，编译时解析
...
```markdown
prompt: >
  @hero 这时想起多年前与 @villain 在 @bar 的相遇 (那次他们订下契约)。
  @hero 此刻面对的是 @temple，但他真正要去的是 @final_battle。
  @brother 此时还在 @sea_beach 不知情况。
```

✗ `@villain` / `@bar` / `@final_battle` / `@brother` / `@sea_beach` 都不在画面里
✗ "想起多年前"、"真正要去的是"、"还在...不知情况" 是叙事不是画面
✗ 模型只会困惑或者尝试把所有元素堆进画面，得到一锅粥

**推荐的拆分**：

| 字段 | 用途 |
|------|------|
| `visual_brief` | 一句话画面摘要（10-30 字） |
| `visual_detailed` | 详细画面描述（构图、光、动作、风格） |
