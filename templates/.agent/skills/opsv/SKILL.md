...
EndCircle     →  动态视频（仅当 shotdeck.md 存在）
```
...
| `shotdeck` | 批量视频生成，末环 EndCircle | 特殊格式；`opsv animate` 编译目标 |
...
> 视频生成不仅限于 shotdeck——任何 category 的文档都可以通过 ComfyUI 工作流（如 4 帧/9 帧配置）生成视频。
