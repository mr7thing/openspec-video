...
# 产出: opsv-queue/videospec_zerocircle_1/imagen_jobs.json
...
# 视频任务（自动推断末端 Circle）
opsv animate
# 产出: opsv-queue/videospec_endcircle_1/video_jobs.json

# ComfyUI 工作流（输出任务描述 JSON，Agent 注入变量后执行）
...
```bash
opsv queue compile opsv-queue/videospec_zerocircle_1/imagen_jobs.json --model volcengine.seadream-5.0-lite --circle zerocircle_1
```
...
# 清单固化 — 已合并到 status，自动写入 .opsv/videospec_manifest.json
```
...
| ✅ | 全部资产已批准 | **允许**启动下一 Circle 的生成管线；manifest 由 `opsv circle status` 自动写入 |
...
opsv animate               # 2. 基于 approved 资产生成视频任务
...
- **manifest 是承诺**：`.opsv/videospec_manifest.json` 是当前状态的快照，由 `opsv circle status` 自动写入。该文件反映最近一次状态扫描结果。
...
| `opsv circle status` | 扫描文档目录，重新计算拓扑排序和批准状态，自动写入 `.opsv/videospec_manifest.json` |
...
| `opsv comfy compile <workflow> --skill <skill>` | ComfyUI 工作流编译为任务描述 JSON，Agent 从技能目录加载 workflow 后注入变量 |
