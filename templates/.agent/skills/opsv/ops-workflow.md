...
# 产出: opsv-queue/videospec/zerocircle/volcengine.seadream/shot_01.json 等
...
# 产出: opsv-queue/videospec/endcircle/volcengine.seedance/shot_01.json 等
...
# 执行多个路径
opsv run opsv-queue/videospec/zerocircle/volcengine.seadream/shot_01.json opsv-queue/videospec/zerocircle/volcengine.seadream/shot_02.json
...
# 再次修改
cp opsv-queue/videospec/zerocircle/volcengine.seadream/@hero_2.json \
   opsv-queue/videospec/zerocircle/volcengine.seadream/@hero_3.json
# 编辑 @hero_3.json
opsv run opsv-queue/videospec/zerocircle/volcengine.seadream/@hero_3.json
# → 生成 @hero_3_1.png
...
- **manifest 是承诺**：`opsv-queue/videospec/_manifest.json` 是当前状态的快照，由 `opsv circle refresh` 自动写入。该文件反映最近一次状态扫描结果。
...
| `opsv circle refresh` | 扫描文档目录，重新计算拓扑排序和批准状态，自动写入 `_manifest.json`（合并原 status + deps） |
| `opsv circle create --dir <path>` | 新建并激活依赖图（支持 `--skip-middle-circle` 简化模式） |
