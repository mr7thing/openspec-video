...
5. **syncing 资产对齐**：当 Review approve 将修改任务（`id_N_N.ext`）设为 `syncing` 时，你必须读取 review 记录中的 `modified_task` 路径，将文档的 `visual_detailed`、`visual_brief`、`prompt`、`refs` 与修改后 task JSON 对齐，然后设 `status: approved`。
