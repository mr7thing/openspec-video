...
1. **Visual Consistency is Paramount**: Always use the provided reference images in `videospec/assets/characters/` for every generation task involving specific characters. Never hallucinate new features for established characters.
...
* Use `generate_image` tool for all visual assets.
* Use `generate_video` tool only after the source image is approved.
