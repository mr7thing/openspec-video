...
// Config, materializes the four Role Context templates (C3, analysis §10.2)
// to `.opsv/bootstrap/roles/<role>.md`, and owns the fail-closed
// `bootstrap_stale` judgement consumed by Execution/hook preflight
// (checkBootstrapStale).
//
...
// ---------------------------------------------------------------------------
// Role Context templates (C3; analysis §10.2)
//
// The templates REFERENCE Pack files by path — they never copy Pack content.
// Everything below is derived from the manifest being written, so a template
// can never point at something the manifest does not also surface.
// ---------------------------------------------------------------------------

function packSkillDoc(projectRoot: string, pack: BootstrapManifest['packs'][number]): string | undefined {
  const abs = path.join(path.resolve(projectRoot, pack.root), 'SKILL.md');
  return fs.existsSync(abs) && fs.statSync(abs).isFile() ? `${pack.root}/SKILL.md` : undefined;
}

function inputSlotLines(manifest: BootstrapManifest): string[] {
  return manifest.io.inputs.map(i =>
    `- ${i.pack}/${i.profile} slot "${i.slot}": category ${i.category}, ref ${i.refType}${i.required ? ' (required)' : ''}`);
}

function outputLines(manifest: BootstrapManifest): string[] {
  return manifest.io.outputs.map(o => `- ${o.pack}/${o.profile} outputs: ${o.outputs.join(', ')}`);
}

function packRootOf(manifest: BootstrapManifest, packId: string): string | undefined {
  return manifest.packs.find(pack => pack.id === packId)?.root;
}

/** Quality guidance references: pack-relative graph.yaml paths joined onto
 *  the Pack root recorded in the manifest (references, never copies). */
function qualityGuidanceLines(manifest: BootstrapManifest): string[] {
  const lines: string[] = [];
  for (const node of manifest.workflowGraph) {
    const guidance = node.stage?.qualityGuidance;
    if (!guidance) continue;
    const root = packRootOf(manifest, node.pack);
    for (const rel of Array.isArray(guidance) ? guidance : [guidance]) {
      const clean = rel.replace(/^\.\//, '');
      lines.push(`- ${node.id} (pack ${node.pack}): ${root ? `${root}/` : ''}${clean}`);
    }
  }
  return lines;
}
...
function section(title: string, lines: string[]): string[] {
  return [`## ${title}`, '', ...(lines.length > 0 ? lines : ['- (none declared)']), ''];
}

/** Render the Role Context template body for one standard role. */
export function renderRoleTemplate(projectRoot: string, role: BootstrapRole, manifest: BootstrapManifest): string {
  const header = [
    `# Role Context: ${role}`,
    '',
    '> Generated by `opsv bootstrap`. This template references Pack files by',
    '> path — read the referenced files for the authoritative content.',
    '',
  ];
  const packDocs = manifest.packs.map(packSkillDoc.bind(null, projectRoot)).filter((ref): ref is string => !!ref);
  const guidanceDocLines = packDocs.map(ref => `- ${ref}`);

  switch (role) {
    case 'document-author':
      return [
        ...header,
        ...section('Document Templates & Section Requirements', manifest.documentContracts.map(c =>
          `- ${c.category} (pack ${c.pack}): ${c.path} — default profile: ${c.defaultProfile ?? '(none)'}; profiles: [${(c.profiles ?? []).join(', ')}]`)),
        ...section('Prompt Syntax (@-ref)', manifest.promptContract.refSyntax.map(form => `- ${form}`)),
        ...section('Examples & Guidance', guidanceDocLines),
      ].join('\n');
    case 'contract-checker':
      return [
        ...header,
        // C4: the role table (spec §Standard Roles) makes contract-checker
        // read-only. State the scope explicitly — and never echo write-
        // directive examples into this template.
        '> Scope: read-only validation — inspect contracts and dependencies,',
        '> emit issue codes. Never write or mutate project state.',
        '',
        ...section('Validation Schemas', [
          ...manifest.documentContracts.map(c => `- ${c.category} (pack ${c.pack}): ${c.path}`),
          ...manifest.packs.map(pack => `- ${pack.id}: ${pack.root}/pack.yaml`),
        ]),
        ...section('Dependency Rules', manifest.workflowGraph.map(node =>
          `- ${node.id} (pack ${node.pack}) depends on: ${node.dependsOn.length > 0 ? node.dependsOn.join(', ') : '(none)'}`)),
        ...section('Input Contracts', inputSlotLines(manifest)),
        ...section('Output Contracts', outputLines(manifest)),
        ...section('Gates', manifest.gates.map(g =>
          `- ${g.pack}/${g.skill}${g.action ? ` (${g.action})` : ''}: ${g.gates.length > 0 ? g.gates.join(', ') : '(none)'}`)),
      ].join('\n');
    case 'production-dispatcher':
      return [
        ...header,
        ...section('Production Inputs', inputSlotLines(manifest)),
        ...section('Production Outputs', outputLines(manifest)),
        ...section('produce/run Rules', [
          ...guidanceDocLines,
          ...manifest.policy.packs.map(p => `- ${p.pack} effective policy: execute=${p.effective.execute ?? '(undeclared)'}`),
        ]),
        ...section('Recommended Provider/API (soft — never a whitelist)', manifest.recommendedCapabilities.map(c =>
          `- ${c.capability} (pack ${c.pack}; profiles: ${c.profiles.join(', ')})${c.binding ? ` — project binding: ${c.binding}` : ''}`)),
      ].join('\n');
    case 'asset-quality-reviewer':
      return [
        ...header,
        ...section('Pack Quality Guidance', [...qualityGuidanceLines(manifest), ...guidanceDocLines]),
        ...section('Review Targets (user-visible)', manifest.workflowGraph
          .filter(node => node.stage?.completion || node.stage?.outputs?.contract)
          .map(node => {
            const completion = node.stage?.completion;
            const contract = node.stage?.outputs?.contract;
            return `- ${node.id} (pack ${node.pack}):${completion ? ` completion [${completion.join(', ')}]` : ''}${contract ? ` output contract: ${contract}` : ''}`;
          })),
      ].join('\n');
  }
}
...
export function writeBootstrap(projectRoot: string): { manifestPath: string; manifest: BootstrapManifest } {
  ...
  // C3: materialize the four Role Context template bodies, then flip the
  // manifest slots from pending to materialized (templates are written before
  // manifest.json so the recorded status always matches the files on disk).
  for (const entry of manifest.roles) {
    fs.writeFileSync(path.join(projectRoot, entry.template), renderRoleTemplate(projectRoot, entry.role, manifest), 'utf8');
