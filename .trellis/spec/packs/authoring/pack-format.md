...
- Binding targets include the `rhcli.*` models (RunningHub via RH_CLI subprocess), which widen the capability surface beyond image/video: `tts`, `music`, `voice-clone`, `upscale`, `video-upscale`, `video-extend`, `motion-control`, `3d-generation`, and `ai-app` (arbitrary ComfyUI cloud workflows). Endpoint ids and param cheat-sheets: `docs/runninghub-capability-map.md`.
